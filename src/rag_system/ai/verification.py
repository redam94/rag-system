...
from .prompts import (
    VERIFY_OUTPUT, VERIFY_PLAN_CHECK, VERIFY_QUICK_CHECK,
    VERIFY_OUTPUT_TEMPLATE, VERIFY_PLAN_CHECK_TEMPLATE, VERIFY_QUICK_CHECK_TEMPLATE,
)
...
class OutputVerifier:
    ...
    def verify(
        ...
        prompt = VERIFY_OUTPUT_TEMPLATE.format(
            query=query,
            plan=plan,
            code=code[:2000] if code else "No code executed",
            stdout=stdout[:2000] if stdout else "No output",
            stderr=stderr[:500] if stderr else "None",
            summary=summary[:1500] if summary else "No summary generated",
            plots_analyzed=", ".join(plots_analyzed) if plots_analyzed else "None",
        )
        ...
        try:
            result = structured_llm.invoke([
                SystemMessage(content=VERIFY_OUTPUT),
    ...
    def check_plan_completion(
        ...
        """
        prompt = VERIFY_PLAN_CHECK_TEMPLATE.format(
            plan=plan,
            code=code[:1500] if code else "None",
            stdout=stdout[:1500] if stdout else "None",
        )
        ...
        try:
            result = structured_llm.invoke([
                SystemMessage(content=VERIFY_PLAN_CHECK),
                HumanMessage(content=prompt),
            ])
    ...
    def quick_check(
        ...
        prompt = VERIFY_QUICK_CHECK_TEMPLATE.format(
            query=query,
        ...
        try:
            response = self.llm.invoke([
                SystemMessage(content=VERIFY_QUICK_CHECK),
                HumanMessage(content=prompt),
            ])
